...
- Three-tier data priority for those percentages, each clearly labelled so a reading is never mistaken for a different kind of number:
  1. **OFFICIAL** - Anthropic's own `rate_limits` figures, captured from Claude Code's status line (see [Reading official rate limits](#reading-official-rate-limits)). Used whenever a reading has ever been captured, however long ago - a badge distinguishes **OFFICIAL · LIVE** (captured moments ago), **OFFICIAL · LAST KNOWN** (older, but that window hasn't reset yet - the number is still correct), and **OFFICIAL · LAST KNOWN** again once that window's reset has passed, now with an "Awaiting refresh" note in place of the (no longer meaningful) reset time - the last known percentage stays on screen either way, never blanked out and never faked as 0%, until a fresh capture replaces it. 5H and Weekly are judged independently, since each has its own reset time. Weekly's reset label also includes the weekday and date (e.g. "Reset Sun 09/14 13:00") since a 7-day-out reset can't be pinned down from the clock time alone; 5H's stays a bare "Reset 14:30".
  2. **ESTIMATED** - a local estimate anchored on a previously-observed rate-limit hit, used only when no official reading has *ever* been captured (no statusLine hook configured, or its capture file has never successfully parsed). Only ever covers a 5-hour-shaped window.
...
`OfficialRateLimitProvider` (`claude_usage_widget/rate_limits.py`) reads that capture file and returns the last successfully captured reading indefinitely - it is never discarded just for being old. Each window's own `resets_at` decides whether it's still shown as current (see the LIVE / LAST KNOWN badges above); only when the capture file has never existed or never parsed does the widget fall back to the estimate/local tiers described above.
